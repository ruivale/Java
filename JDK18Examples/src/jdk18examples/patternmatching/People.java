...
/**
 * <p>
 * Description: The guard in pattern line [2] seems like it would match Kane at age 118, but instead
 * Kane matches with the pattern at line [1]. You cannot rely on the compiler to help with the logic
...
public class People {
  ...
  static String categorize(Person person) {
    return switch (person) {
      case Person p
         && p.age() > 40 -> p + " is middle aged";                           // [1]
      case Person p
         && (p.name().contains("D") || p.age() == 14) -> p + " D or 14";
      case Person p
         && !(p.age() >= 100) -> p + " is not a centenarian";                // [2]
      case Person p ->
        p + " Everyone else";
    };
  }
  ...
  /**
   * Output: Person[name=Dorothy, age=15] D or 14 Person[name=John Bigboote, age=42] is middle aged
   * Person[name=Morty, age=14] D or 14 Person[name=Morty Jr., age=1] is not a centenarian
   * Person[name=Jose, age=39] is not a centenarian Person[name=Kane, age=118] is middle aged
   *
   * @param args
   */
