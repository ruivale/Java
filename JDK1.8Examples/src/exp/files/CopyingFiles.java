...
public class CopyingFiles {
  ...
  public static void renamingAndMovingFiles(final String[] args) {
    try {
  ...
  /**
   *
   * @param fileOriginal File
   * @param fileDestinationDir File
   */
  private static void copyFiles(
      final File fileOriginal,
      final File fileDestinationDir) throws Exception {

    final File fileDest = new File(
      new StringBuffer(fileDestinationDir.getAbsolutePath()).append("/").append(
          fileOriginal.getName()).toString());

    //if(fileDest.exists()){
      //fileDest.delete();
   // }

    final FileInputStream in = new FileInputStream(fileOriginal);
    final FileOutputStream out = new FileOutputStream(fileDest);
    int c;
