...
public class SliderSnap
  ...
  public static ComponentUI createUI(JComponent c) {
    ...
    try {
      ...
      if (m == null) {
        m = sliderClass.getMethod("createUI", new Class[]{JComponent.class});
  ...
  /**
   * Listeners for when the JSlider becomes visible then attaches the mouse listeners, then removes
   * itself.
   */
  private static class MouseAttacher implements HierarchyListener {
    ...
    public void hierarchyChanged(HierarchyEvent evt) {
      ...
      if ((flags & HierarchyEvent.DISPLAYABILITY_CHANGED) > 0 && evt.getComponent() instanceof JSlider) {
  ...
  private static class Init implements Runnable {
    ...
    public void run() {
      try {
        ...
        xForVal = BasicSliderUI.class.getDeclaredMethod("xPositionForValue", new Class[]{int.class});
        yForVal = BasicSliderUI.class.getDeclaredMethod("yPositionForValue", new Class[]{int.class});
  ...
  private static class SnapListener extends MouseInputAdapter implements PropertyChangeListener {
    ...
    /**
     * Implements the actual "snap while dragging" behaviour. If snap to ticks is enabled on this slider,
     * then the location for the nearest tick/label is calculated and the click location is translated
    ...
    /**
     * When the slider is clicked we need to record the offset from thumb center.
     */
    ...
    /**
     * Calculates the nearest snapable value given a MouseEvent. Code adapted from BasicSliderUI.
     */
    ...
    /**
     * Provides the x or y co-ordinate for a slider value, depending on orientation.
     */
    public int getLocationForValue(int value) {
      try {
        // Reflectively call slider ui code
        Method m = slider.getOrientation() == JSlider.HORIZONTAL? xForVal : yForVal;
        Integer result = (Integer) m.invoke(ui, new Object[]{new Integer(value)});
