...
public class MultiBufferTest {
  ...
  private static Color[] COLORS = new Color[] {
      ...
      Color.gray, Color.cyan, Color.pink, Color.lightGray, Color.magenta,
  ...
  /**
   * Creates a new MultiBufferTest object.
   *
    ...
    try {
      ...
      if((args != null) && (args.length > 0)) {
        ...
        if((numBuffers < 2) || (numBuffers > COLORS.length)) {
          System.err.println(
            "Must specify between 2 and " + COLORS.length + " buffers");
          System.exit(1);
        }
      }
  ...
  private static DisplayMode getBestDisplayMode (GraphicsDevice device) {
    for(int x = 0; x < BEST_DISPLAY_MODES.length; x++) {
      ...
      for(int i = 0; i < modes.length; i++) {
        if(
          (modes[i].getWidth() == BEST_DISPLAY_MODES[x].getWidth()) &&
