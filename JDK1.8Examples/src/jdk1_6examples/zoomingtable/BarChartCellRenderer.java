package jdk1_6examples.zoomingtable;
 ...
 * @author thierryl
 */
public class BarChartCellRenderer
    extends DefaultTableCellRenderer implements TableModelListener {

  /** Array of Colors used
   * For now the only used color is BLUE*/
  private static final Color[] COLORS = new Color[] {new Color(0x002F60)};
  /**
  ...
  private void resetMinMax(int iColumn, Class iColumnClass) {
    ...
    for (int i = 0; i < lRowCount; i++) {
      ...
      try {
        ...
        int lPreferredWidth = getPreferredWidthForValue(lValue, iColumn,
            iColumnClass);
  ...
  private int getPreferredWidthForValue(double iValue, int iColumnIndex,
      Class iColumnClass) {
    Format lFormat = getFormat(new Integer(iColumnIndex), iColumnClass);
    JLabel lLabel = new JLabel();
    lLabel.setFont(getFont());
    lLabel.setText(lFormat == null ? Double.toString(iValue) :
   ...
   */
  public Component getTableCellRendererComponent(JTable iTable, Object iValue,
      boolean iIsSelected, boolean iHasFocus, int iRow, int iColumn) {
    ...
    Component lComp = super.getTableCellRendererComponent(iTable, iValue,
        iIsSelected, iHasFocus, iRow, iColumn);

    if (lComp instanceof JLabel) {
      double lValue = getDoubleFromValue(iValue);
      int lColumnWidth = iTable.getColumnModel().getColumn(iColumn).getWidth();
      ...
      if (isRenderingEnabled()) {
        MinMaxPair lMinMaxPair = (MinMaxPair) mMinMaxMap.get(lColumn);
        ...
        int lLabelWidth = ( (MinMaxPair) mMinMaxMap.get(lColumn)).
            mLabelPreferredWidth;
        lLabelWidth += getIconTextGap() * 2;
        BarChartIcon lIcon = new BarChartIcon(lColumnWidth - lLabelWidth,
            lRowHeight, 0, lMin, lMax, lValue, 0,
  ...
  private double getMaximumRenderedColumns(Class iClass) {
    ...
    while (lIter.hasNext()) {
      ...
      if (lColumnClass.equals(iClass)) {
        lRes = Math.max( ( (MinMaxPair) mMinMaxMap.get(lColumn)).mMax, lRes);
      }
    }
    return lRes;
  }
  ...
  private double getMinimumRenderedColumns(Class iClass) {
    ...
    while (lIter.hasNext()) {
      ...
      Class lColumnClass = (Class) mColumnClassMap.get(lColumn);
      if (lColumnClass.equals(iClass)) {
        lRes = Math.min( ( (MinMaxPair) mMinMaxMap.get(lColumn)).mMin, lRes);
      }
    }
    return lRes;
  }
  ...
  public void setFont(Font iFont) {
    ...
    while (lIter.hasNext()) {
      ...
      lPair.mLabelPreferredWidth = getPreferredWidthForValue(lPair.
          mLabelPreferredValue, lColumnIndex.intValue(), lColumnClass);
    }
