...
public class ZoomWheelListener
    extends MouseAdapter implements MouseWheelListener, ListSelectionListener {
   ...
   */
  private static Class[] DEFAULT_RENDERED_CLASS = {Object.class, Number.class,
      Float.class, Double.class, Date.class, Icon.class, ImageIcon.class,
      Boolean.class};
  ...
  private static final class ZoomRenderer
    ...
    public Component getTableCellRendererComponent(JTable table, Object value,
      ...
      Component lComp = mRendererDelegate.getTableCellRendererComponent(table,
  ...
  public static void installZoomRenderer(JTable iTable) {
    ...
    for (int i = 0; i < DEFAULT_RENDERED_CLASS.length; i++) {
      TableCellRenderer lRenderer = iTable.getDefaultRenderer(
          DEFAULT_RENDERED_CLASS[i]);
      if (lRenderer != null && ! (lRenderer instanceof ZoomRenderer)) {
        iTable.setDefaultRenderer(DEFAULT_RENDERED_CLASS[i],
  ...
  public static void unInstallZoom(JTable oTable) {
    MouseWheelListener[] lWheelListeners = oTable.getMouseWheelListeners();
    ...
    MouseListener[] lMouseListeners = oTable.getMouseListeners();
    ...
    PropertyChangeListener[] lZoomModelListeners = oTable.
        getPropertyChangeListeners("model");
    for (int i = 0; i < lZoomModelListeners.length; i++) {
      if (lZoomModelListeners[i] instanceof ZoomTableModelListener) {
        oTable.removePropertyChangeListener("model", lZoomModelListeners[i]);
      }
    }
  }
  ...
  public void mouseClicked(MouseEvent iEvent) {
    if (iEvent.getClickCount() == 2 && iEvent.isControlDown() &&
