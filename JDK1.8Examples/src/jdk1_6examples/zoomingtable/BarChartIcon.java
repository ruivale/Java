...
public class BarChartIcon
   ...
   */
  public BarChartIcon(int iWidth, int iHeight, int iOffset, double iMin,
      double iMax, double iValue, double iZero) {
    mMaxWidth = iWidth;
    mHeight = iHeight;
    mMin = iMin;
    mMax = iMax;
    ...
    mOffset = iOffset;
    mZero = iZero;
    mWidth = (int) ( ( (Math.max(iValue - iZero, iZero) - iMin) / (iMax - iMin)) *
                    iWidth);
  }
   ...
   */
  public BarChartIcon(int iWidth, int iHeight, int iOffset, double iMin,
      double iMax, double iValue, double iZero, Color iFillColor) {
  ...
  public void paintIcon(Component iComp, Graphics oGraphics, int iX, int iY) {
    ...
    int lZeroX = (int) ( ( (mZero - mMin) / lVirtualWidth) * (mMaxWidth) -
                        mOffset);
    int lBarWidth = (int) Math.abs( ( (mValue - mZero) / lVirtualWidth) *
                                   (mMaxWidth - 2 * mOffset));
...
//			((Graphics2D) oGraphics).setPaint(lPaint);
      oGraphics.fillRect(lX, lY, lBarWidth, lHeight);
//			((Graphics2D) oGraphics).setPaint(lReversePaint);
      oGraphics.drawRect(lX, lY, lBarWidth, lHeight);
    } else {
      lX = lZeroX - lWidth;
//			((Graphics2D) oGraphics).setPaint(lPaint);
      oGraphics.fillRect(lX, lY, lWidth, lHeight);
//			((Graphics2D) oGraphics).setPaint(lReversePaint);
      oGraphics.drawRect(lX, lY, lWidth, lHeight);
    }
    if (mFormater != null) {
      String lText = null;
      try {
      ...
      if (lText != null) {
        int lTextWidth = ( (Graphics2D) oGraphics).getFontMetrics().stringWidth(
            lText);
        //Stay in Icon Width
        int lStartText = 0;
        if (lTextWidth < mMaxWidth) {
          lStartText = lZeroX + 10 + lTextWidth > mMaxWidth ?
              mMaxWidth - 10 - lTextWidth : lZeroX + 10;
        }
        oGraphics.drawString(lText, lStartText, lY + lHeight - mOffset);
