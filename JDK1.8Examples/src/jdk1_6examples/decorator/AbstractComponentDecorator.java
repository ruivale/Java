...
public abstract class AbstractComponentDecorator {
  ...
  protected void attach() {
    ...
    if (w instanceof RootPaneContainer) {
      ...
      if (this instanceof BackgroundPainter) {
        ...
        if (layerOffset < 0) {
          BackgroundPainter bp = (BackgroundPainter) lp.getClientProperty(
              BackgroundPainter.key(base));
          if (bp == null) {
  ...
  private Rectangle getVisibleRect(JComponent component) {
    ...
    if (visible.x != 0 || visible.y != 0 || visible.width != component.getWidth() ||
  ...
  private static class BackgroundPainter
    ...
    public void paint(Graphics g) {
      ...
      for (Iterator i = components.iterator(); i.hasNext(); ) {
        JComponent c = (JComponent) i.next();
        paintForeground(g.create(c.getX(), c.getY(), c.getWidth(), c.getHeight()),
                        c);
      }
    }
  ...
  private final class Listener
      extends ComponentAdapter implements HierarchyListener,
