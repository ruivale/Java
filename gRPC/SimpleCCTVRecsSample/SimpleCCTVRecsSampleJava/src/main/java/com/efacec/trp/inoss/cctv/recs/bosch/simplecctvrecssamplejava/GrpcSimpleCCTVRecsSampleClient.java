/**
 * <p>
 * Classname:  com.efacec.trp.inoss.cctv.recs.bosch.simplecctvrecssamplejava.GrpcSimpleCCTVRecsSampleClient
 * </p>
 *
...
package com.efacec.trp.inoss.cctv.recs.bosch.simplecctvrecssamplejava;
...
import io.grpc.examples.simplecctvrecs.ObtainRecsServiceGrpc;
import io.grpc.examples.simplecctvrecs.ObtainRecsServiceGrpc.ObtainRecsServiceBlockingStub;
import io.grpc.examples.simplecctvrecs.ObtainRecsServiceGrpc.ObtainRecsServiceStub;
import io.grpc.examples.simplecctvrecs.RecsRequest;
import io.grpc.examples.simplecctvrecs.RecsReply;
 ...
 * @since __DATE__
 */
public class GrpcSimpleCCTVRecsSampleClient {
  /** This class LOGGER */
  private static final Logger LOGGER =
    Logger.getLogger(GrpcSimpleCCTVRecsSampleClient.class.getName());

  private ObtainRecsServiceBlockingStub blockingStub;
  private ObtainRecsServiceStub nonBlockingStub;
  
  private List<RecsRequest> listObtainrecsRequest = 
    Arrays.asList(
      RecsRequest.newBuilder().setTrackId(2).setSourceId(2).setSourceUrl("SourceUrl 2").setStartTimeT(22222222).setEndTimeT(22222222).build(),
      RecsRequest.newBuilder().setTrackId(3).setSourceId(3).setSourceUrl("SourceUrl 3").setStartTimeT(33333333).setEndTimeT(33333333).build(),
      RecsRequest.newBuilder().setTrackId(4).setSourceId(4).setSourceUrl("SourceUrl 4").setStartTimeT(44444444).setEndTimeT(44444444).build(),
      RecsRequest.newBuilder().setTrackId(6).setSourceId(6).setSourceUrl("SourceUrl 6").setStartTimeT(66666666).setEndTimeT(66666666).build(),
      RecsRequest.newBuilder().setTrackId(9).setSourceId(9).setSourceUrl("SourceUrl 9").setStartTimeT(99999999).setEndTimeT(99999999).build()      
    );
  
  
 /**
  * The GrpcSimpleCCTVRecsSampleClient default constructor.
  ...
  * 
  */
  public GrpcSimpleCCTVRecsSampleClient(final ManagedChannel channel){
   ...
   */
  private void obtainRecs() throws InterruptedException {

    final RecsRequest request = 
      RecsRequest.newBuilder()
        .setTrackId(9999)
        .setSourceId(999)
    ...
    try {
      ...
      response = this.blockingStub.obtainRec(request);

      System.out.println("Client.obtainRecsServerSideStreaming() RESPONSE : " + Utils.toString(response));
...
//        .build();
//
//    Iterator<RecsReply> responses;
//    
//    try {
//      System.out.println("Client.obtainRecsServerSideStreaming() REQUEST: " + Utils.toString(request));
//
//      responses = this.blockingStub.obtainRec(request);
//
//      for (int i = 1; responses.hasNext(); i++) {
//        final RecsReply response = responses.next();
//        
...
//    
//    StreamObserver<RecsReply> responseObserver = new StreamObserver<RecsReply>() {
//      @Override
//      public void onNext(final RecsReply response) {
...
//
//    
//    final StreamObserver<RecsRequest> requestObserver = 
//        this.nonBlockingStub.obtainRec(responseObserver);
//    
//    try {
//      for (RecsRequest request : this.listObtainrecsRequest) {
...
//    StreamObserver<RecsReply> responseObserver = new StreamObserver<RecsReply>() {
//      @Override
//      public void onNext(RecsReply response) {
...
//    };
//
//    StreamObserver<RecsRequest> requestObserver = 
//        this.nonBlockingStub.obtainRec(responseObserver);
//    
//    try {
//      for (RecsRequest request :  this.listObtainrecsRequest) {
...
//  
  ...
  public static void main(String[] args) throws Exception {
    ...
    final ManagedChannel channel = 
        ManagedChannelBuilder.forAddress("localhost", 30051)
            .usePlaintext()
            .build();

    try {
       
      GrpcSimpleCCTVRecsSampleClient client = new GrpcSimpleCCTVRecsSampleClient(channel);
